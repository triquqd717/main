...
Frame.ChildAdded:Connect(function(notif)
	if notif.Name == "Notification_UI" then
		...
		if Label then
			log("Notification received:", Label.Text)
			if not string.find(Label.Text, "<font") or string.find(Label.Text, "was restocked") then
				...
				if not check(clean) then
					...
					log("Cleaned text:", Final)
					send(Final)
				end
			else
				log("Skipped rich-text notification:", Label.Text)
			end
		else
			log("No TextLabel found inside Notification_UI")
		end
	end
end)
...
local function Main()
	...
	_G.WeatherThread = task.spawn(function()
		...
		GameEvents.WeatherEventStarted.OnClientEvent:Connect(function(EventType, EventDuration)
			...
			local EndTime = math.floor(os.time() + EventDuration)
